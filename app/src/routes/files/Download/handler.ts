...
const DownloadFileHandler:  Handler<APIGatewayProxyEvent, APIGatewayProxyResult>  = async (event) => {
    ...
    const data = await s3.generateDownloadPreSignedUrl(`${process.env.BUCKET_NAME}-${process.env.STAGE}`, fileKey, expiresIn as number | undefined);

    return response({ signedUrl: data }, 200);
};

export const handler = middy(DownloadFileHandler).use(catchFinallyHandler());